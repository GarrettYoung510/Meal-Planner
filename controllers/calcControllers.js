...
getBMR: async (req, res) =>{
    try {
        ...
        console.log(user);
        const { email,first_name, last_name, age, gender, weight, height, activity_level} = user;
        res.json({email, first_name, last_name, age, gender, weight, height, activity_level})
    } catch (e) {
        res.json(e)
    }
}
