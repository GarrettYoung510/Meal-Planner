...
module.exports = {
  signUp: async (req, res) => {
    console.log();
    const { email, password, first_name, last_name, age, gender, weight, height, activity_level} = req.body ;
    if(!email || !password) {
    ...
    try {
      ...
      if(gender === 'female'){
        const femaleBMR = 655 + (4.3 * parseInt(weight)) + (4.7 * parseInt(height)) - (4.7 * parseInt(age));
        ...
        if(parseInt(activity_level) === 1) {
            ...
            const user = new db.User({ email, password, first_name, last_name, age, gender, weight, height, activity_level, fat, protein, carb, calories });        
            await user.save();
      ...
      if(gender === 'male'){
        ...
        const maleBMR = 66 + (6.3 * parseInt(weight)) + (12.9 * parseInt(height)) - (6.8 * parseInt(age));
        if(parseInt(activity_level) === 1) {
